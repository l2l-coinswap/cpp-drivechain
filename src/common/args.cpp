...
const char * const BITCOIN_CONF_FILENAME = "drivechaincpp.conf";
...
{
    // Windows:
    //   old: C:\Users\Username\AppData\Roaming\drivechaincpp
    //   new: C:\Users\Username\AppData\Local\drivechaincpp
    // macOS: ~/Library/Application Support/drivechaincpp
    // Unix-like: ~/.drivechaincpp
#ifdef WIN32
    // Windows
    // Check for existence of datadir in old location and keep it there
    fs::path legacy_path = GetSpecialFolderPath(CSIDL_APPDATA) / "drivechaincpp";
    if (fs::exists(legacy_path)) return legacy_path;

    // Otherwise, fresh installs can start in the new, "proper" location
    return GetSpecialFolderPath(CSIDL_LOCAL_APPDATA) / "drivechaincpp";
#else
    fs::path pathRet;
...
#ifdef __APPLE__
    // macOS
    return pathRet / "Library/Application Support/drivechaincpp";
#else
    // Unix-like
    return pathRet / ".drivechaincpp";
#endif
#endif
}
